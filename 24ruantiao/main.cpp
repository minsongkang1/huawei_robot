...
int select_dist[11][N][N], select_path[11][N][N], dist_num[11], case_dist[N][N], case_path[N][N], countpp;
int tmppath[N][N];
...
void Init()
{
    ...
    if (countpp == 0)
    {
        dist_pre(4, case_dist, case_path);
        ch[107][0] = '#';
    }
...
bool dfs(int id, int drc, int x, int y, int px, int py)
{
    ...
    {
        if (robot[robot_in_graph[px][py]].goods == 1)
            return false;
        ...
        for (int j : ans)
        {
            ...
            {
                //   if( robot_in_graph[tx][ty] != -1 && robot[robot_in_graph[tx][ty]].goods == 1) continue;
...
void robot_action()
{
    if (!fal && countpp == 0)
    {
        ...
        if (ctt != 5)
        {
            int x = robot[4].x, y = robot[4].y;
            int dr = case_path[x][y];
            int px = x + dx[dr], py = y + dy[dr];
            move_get(4, dr, x, y, px, py);
            x = robot[7].x, y = robot[7].y;
            dr = case_path[x][y];
            px = x + dx[dr], py = y + dy[dr];
            move_get(7, dr, x, y, px, py);
        }
        else
            fal = true;
    }
...
{
    if (countpp == 0 && zhen <= 10000)
        return;
    else if (countpp != 0 && zhen <= 11600)
        return;
    for (int i = 0; i < 10; i++)
    {
...
// double boat_go_threshold[5] = {0.75,0.75,0.75,0.75,0.75}; // 船后期尽量多装 -> 调整此值
...
{
    int c = 15000;
    double go_threshold = 0.8;
    int deadline = 11600;
    if (countpp == 0)
    {
        random_device rd;
        int random_number = abs((int)rd()) % 30 + 50;
        go_threshold = (double)random_number / 100;
        deadline = 11000;
    }

    fprintf(file, "########%d########\n", zhen);
    fprintf(file, "total_money:%d\tmoney:%d\n", total_money, money);
    ...
    for (int o = 0; o < berth_num; o++)
    {
        ...
        fprintf(file, "berth_ID:%d\tadd_num : %d\tberth_num:%lf\tberth_temp:%d\n", o, addnum[o], berth[o].goods_num, temp_value);
    }

    //   if(zhen==1){
    //		// 按最短时间去
    //		vector<pair<int, int>> berth_sort;
    //		for(int j=0; j<berth_num; j++){
    //			berth_sort.push_back(make_pair(berth[j].transport_time,j));
    //		}
    //		sort(berth_sort.begin(),berth_sort.end());//虚拟点到港口耗时升序
    //		for (int i = 0; i < 5; i++){
    //			boat[i].pre_pos = boat[i].pos;
    //	        boat[i].cur_num = 0;
    //	        boat[i].pos = berth_sort[i].second;
    ...
    // 随机去
    //    if(zhen==1){
    //        int rand_id = 0; // 随机港口id 0 2 4 6 8
    //        for (int i = 0; i < 5; i++){
    //            boat[i].pre_pos = boat[i].pos;
    //            boat[i].cur_num = 0;
    //            boat[i].pos = rand_id;
            ...
            else
            {
                ...
                { // 货物未到70%,寻找下一个港口
                    for (int j = 0; j < berth_num; j++)
                    {
                        ...
                        {
                            temp_score = 500 - weigh1 * ((goods_num - temp_value) - boat_temp);
                        ...
                        {
                            temp_score = 500 + weigh2 * ((goods_num - temp_value) - boat_temp);
                            time_cost = (((goods_num - temp_value) / loading_speed) + (0.5 * 500));
                        }
